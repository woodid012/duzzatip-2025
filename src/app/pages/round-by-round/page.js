  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-8 overflow-x-auto">
        ...
        <table className="min-w-full divide-y divide-slate-100 bg-white rounded-2xl border border-slate-200/80 shadow-sm">
          <thead className="bg-slate-50/80">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Rank</th>
              <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Player</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Played</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">W-L-D</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Player Pts</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Dead Cert</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Total</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">PF</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">PA</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">%</th>
            </tr>
          </thead>
              ...
              return (
                <tr key={userId} className={index === 0 ? 'bg-yellow-50' : ''}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm font-bold ${index === 0 ? 'text-yellow-600' : 'text-slate-900'}`}>
                      {index + 1}
                      {index === 0 && ' 👑'}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-center">
                    <span className={seasonData.deadCertScore >= 0 ? 'text-emerald-600' : 'text-red-600'}>
      ...
      <div className="overflow-x-auto">
        ...
        <table className="min-w-full divide-y divide-slate-100 bg-white rounded-2xl border border-slate-200/80 shadow-sm">
          <thead className="bg-slate-50/80">
            <tr>
              <th rowSpan="2" className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider sticky left-0 bg-slate-50/80 z-10">Player</th>
              {[...Array(21)].map((_, i) => (
                <th key={i + 1} colSpan="4" className="px-2 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider border-l">
                  R{i + 1}
                </th>
              ))}
              <th rowSpan="2" className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider border-l">Season Total</th>
            </tr>
            <tr>
              {[...Array(21)].map((_, i) => (
                <React.Fragment key={i}>
                  <th className="px-1 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">PS</th>
                  <th className="px-1 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">DC</th>
                  <th className="px-1 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Tot</th>
                  <th className="px-1 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">W/L</th>
                </React.Fragment>
              ))}
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-slate-100">
            {sortedUsers.map(([userId, userName], userIndex) => (
              <tr key={userId} className={userIndex === 0 ? 'bg-yellow-50' : ''}>
                ...
                {[...Array(21)].map((_, i) => {
                  ...
                  return (
                    <React.Fragment key={round}>
                      <td className={`px-1 py-4 whitespace-nowrap text-xs text-center border-l ${!hasData ? 'text-slate-300' : 'text-slate-700'}`}>
                        {hasData ? roundScores.playerScore || 0 : '-'}
                      </td>
                      <td className={`px-1 py-4 whitespace-nowrap text-xs text-center ${
                        !hasData ? 'text-slate-300' :
                        (roundScores.deadCertScore || 0) >= 0 ? 'text-emerald-600' : 'text-red-600'
                      }`}>
                        {hasData ? (
                          <>
                      ...
                      <td className={`px-1 py-4 whitespace-nowrap text-xs font-bold text-center ${
                        !hasData ? 'text-slate-300' : 'text-slate-900'
                      }`}>
                      ...
                      <td className={`px-1 py-4 whitespace-nowrap text-xs font-medium text-center ${
                        !hasData ? 'text-slate-300' :
                        roundScores.matchResult === 'W' ? 'text-emerald-600' :
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-900 text-center border-l">
