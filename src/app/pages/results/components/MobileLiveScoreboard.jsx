...
export default function MobileLiveScoreboard({
  ...
  return (
    <div className="block sm:hidden -mx-4 px-4 pb-10 pt-2 text-slate-700">
      {/* Top row: view toggle (grows) + round selector + refresh, all on one line */}
      <div className="flex items-center gap-2 mb-4">
        <div className="flex flex-1 min-w-0 gap-1 rounded-[13px] border border-slate-200 bg-slate-100 p-1">
          <button
            onClick={() => setMode('scores')}
            className={`flex-1 rounded-[10px] py-2 text-[12px] font-extrabold transition-colors ${
        ...
        {isLoggedIn && onRoundChange && (
          <select
            ...
            className="dz-select shrink-0 py-1.5 text-sm"
          >
            {[...Array(25)].map((_, i) => (
        ...
        {onRefresh && (
          <button
            ...
            className="inline-flex shrink-0 items-center justify-center rounded-lg border border-slate-300 bg-white p-2 text-slate-600 shadow-sm transition-colors hover:bg-slate-50 disabled:opacity-60"
          >
