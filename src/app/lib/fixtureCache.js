...
async function overlayAflScores(fixtures, year) {
  ...
  // Build a combined score map across all started rounds, and note which
  // local rounds currently contain a LIVE match — for those, kick off a
  // player-stats refresh even though no final score has landed yet (the
  // AFL API omits totalScore until status flips to CONCLUDED).
  ...
  if (year === CURRENT_YEAR) {
    ...
    if (needsScore.length > 0) {
      try {
        ...
        // Kick off a player-stats refresh for any round where either (a) a
        // fixture's final scores newly landed, or (b) a match is currently
        // LIVE — the AFL API only publishes totalScore once status flips to
        // CONCLUDED, so we can't rely on (a) alone to catch in-progress games.
        // Best-effort, fire-and-forget; refreshGameResultsForRound throttles
        // itself at 5 minutes per round.
        if (year === CURRENT_YEAR) {
